...
public class DecimalDelimiterScaleHandler implements TypeDetailInterface {
    static final Log LOG = LogFactory.getLog(DecimalDelimiterScaleHandler.class);

    //(U'\u0007',0)
    //('\007'.2)
    static final String patterns[] =
            {
                    "^U?(['\"])(\\\\[uU][0-9A-F]{1,4})\\1 *([,\\.]) *([0-9]{1,3}).*$",
                    "^(['\"])(\\\\[xX][0-9A-F]{2})\\1 *([,\\.]) *([0-9]{1,3}).*$",
                    "^(['\"])(\\\\[0-7]{3})\\1 *([,\\.]) *([0-9]{1,3}).*$",
                    "^U?(['\"])(\\\\[abfnrtv])\\1 *([,\\.]) *([0-9]{1,3}).*$",
                    "^(['\"])(.{1,3})\\1 *([,\\.]) *([0-9]{1,3}).*$"
            };
    ...
    public Boolean match(String detail) {
        if (detail == null)
            ...
            return !StringUtils.containsIgnoreCase(detail, "maximum_length") &&
                    (detail.matches(patterns[0]) ||
    ...
    public void parseTypeDetail(String detail, AbColumnDef abColumnDef) {
        ...
        if (found) {
            ...
            if (scale.compareTo(38) > 0)
                LOG.warn(abColumnDef.getName() + ": Decimal scale[" + scale + "] is defined more than 38");
