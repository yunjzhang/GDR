...
public class DecimalDelimiterLengthScaleHandler implements TypeDetailInterface {
    static final Log LOG = LogFactory.getLog(DecimalDelimiterLengthScaleHandler.class);

    //(U'\u0007',0,maximum_length=18)
    //('\007'.2,maximum_length=18)
    static final String patterns[] =
            {
                    "^U?(['\"])(\\\\[uU][0-9A-F]{1,4})\\1 *([,\\.]) *([0-9]{1,3}) *(,.*)*, *maximum_length *= *([0-9]{1,3})(,.*)*$",
                    "^(['\"])(\\\\[xX][0-9A-F]{2})\\1 *([,\\.]) *([0-9]{1,3}) *(,.*)*, *maximum_length *= *([0-9]{1,3})(,.*)*$",
                    "^(['\"])(\\\\[0-7]{3})\\1 *([,\\.]) *([0-9]{1,3}) *(,.*)*, *maximum_length *= *([0-9]{1,3})(,.*)*$",
                    "U?^(['\"])(\\\\[abfnrtv])\\1 *([,\\.]) *([0-9]{1,3}) *(,.*)*, *maximum_length *= *([0-9]{1,3})(,.*)*$",
                    "^(['\"])(.{1,3})\\1 *([,\\.]) *([0-9]{1,3}) *(,.*)*, *maximum_length *= *([0-9]{1,3})(,.*)*$"
            };
    ...
    public void parseTypeDetail(String detail, AbColumnDef abColumnDef) {
        ...
        if (found) {
            ...
            Long length = Long.valueOf(matcher.group(6));
            ...
            Integer scale = Integer.valueOf(matcher.group(4));
            if (scale.compareTo(38) > 0)
                LOG.warn(abColumnDef.getName() + ": Decimal scale[" + length + "] is defined more than 38");
            ((AbDecimalColumnDef) abColumnDef).setScale(scale);

            String decimalType = matcher.group(3);
            if (decimalType.equals("."))
                ((AbDecimalColumnDef) abColumnDef).setDecimalType(DecimalType.EXPLICIT);
            else
                ((AbDecimalColumnDef) abColumnDef).setDecimalType(DecimalType.IMPLICIT);
        } else {
