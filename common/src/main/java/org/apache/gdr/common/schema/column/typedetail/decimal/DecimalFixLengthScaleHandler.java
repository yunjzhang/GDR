...
public class DecimalFixLengthScaleHandler implements TypeDetailInterface {
    static final Log LOG = LogFactory.getLog(DecimalFixLengthScaleHandler.class);

    //(18,2)
    //(18.2)
    private String pattern = "^([0-9]{1,3}) *([,\\.]) *([0-9]{1,3}) *(,.*)*";
    ...
    public void parseTypeDetail(String detail, AbColumnDef abColumnDef) {
        ...
        if (detail.matches(pattern)) {
            ...
            if (matcher.matches()) {
                ...
                Integer scale = Integer.valueOf(matcher.group(3));
                ...
                String decimalType = matcher.group(2);
